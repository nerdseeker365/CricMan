...
//============================ F.C COMPAIR ====================
 if((strcmp($mtype,"fc"))==0){	
	...
	$sql = "select * from solo_fc s where s.uid=$uid and s.play_id=$pid";
	echo '<h2> F.C : '.$sql.'</h2>';
...
else if(strcmp($mtype,"test")==0){
	
		 
	$sql = "select * from solo_test s where s.uid=$uid and s.play_id=$pid";
	echo '<h2>TEST : '.$sql.'</h2>';
	$result = mysqli_query ( $conn, $sql );	 
	if (mysqli_num_rows ( $result ) > 0) {
	$row = mysqli_fetch_assoc($result);
...
$sqlupdate = "UPDATE solo_test SET 
mat ='$mat',	
bat_inngs ='$bat_ing',	
nos ='$nos',	
balls ='$balls', 	
...
}else if(strcmp($mtype,"odi")==0){
	
		$sql = "select * from solo_odi s where s.uid=$uid and s.play_id=$pid";
	echo '<h2>ODI : '.$sql.'</h2>';
	$result = mysqli_query ( $conn, $sql );	 
	if (mysqli_num_rows ( $result ) > 0) {
	$row = mysqli_fetch_assoc($result);
...
$status = "Data NOT Saved";
 ...
 //========================== END CODE ==================================
}else if(strcmp($mtype,"tt")==0){
	
	$sql = "select * from solo_tt s where s.uid=$uid and s.play_id=$pid";
	echo '<h2>T20 : '.$sql.'</h2>';
	$result = mysqli_query ( $conn, $sql );	 
	if (mysqli_num_rows ( $result ) > 0) {
	$row = mysqli_fetch_assoc($result);
